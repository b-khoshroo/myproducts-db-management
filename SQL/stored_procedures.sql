...
AS 
BEGIN
-- Inserting a part of the information into the customer table:
...
END TRY
BEGIN CATCH
        PRINT 'Error in insertion: ' + ERROR_MESSAGE();
        RETURN -1;
END CATCH

-- Inserting the information into the customer table:
INSERT INTO customer(name, address)
VALUES(@cus_name, @cus_add);
...
AS
BEGIN
    IF (@year not in (select YEAR(product.date_produced) from product))
        PRINT 'No product was made in the given year.'
    DECLARE @avg_cost1 REAL,
            @avg_cost2 REAL,
            @avg_cost3 REAL;
    --calculating the average cost of all products of type 1 made in the given year:
    SET @avg_cost1 = (SELECT AVG(product_cost)
                        FROM product1_account, product 
                        WHERE product1_account.product_id=product.id AND YEAR(product.date_produced)=@year);

    --calculating the average cost of all products of type 2 made in the given year:
    SET @avg_cost2 = (SELECT AVG(product_cost)
                        FROM product2_account, product 
                        WHERE product2_account.product_id=product.id AND YEAR(product.date_produced)=@year);

    --calculating the average cost of all products of type 3 made in the given year:
    SET @avg_cost3 = (SELECT AVG(product_cost)
                        FROM product3_account, product 
                        WHERE product3_account.product_id=product.id AND YEAR(product.date_produced)=@year);
    
    --returning the overall average:
    RETURN ((@avg_cost1+@avg_cost2+@avg_cost3)/3);
END
GO*/
...
/*CREATE PROCEDURE q14_avg_cost
    @year INT
AS
-- Cheking if any prodect has been made in the given year:
BEGIN
    IF (@year not in (select YEAR(product.date_produced) from product))
    BEGIN
        PRINT 'No product was made in the given year.';
    ...
    --calculating the average cost of all products of type 1 made in the given year:
    IF ((SELECT AVG(product_cost)
                        FROM product1_account, product 
                        WHERE product1_account.product_id=product.id AND YEAR(product.date_produced)=@year) is not null)
    BEGIN 
    SET @avg_cost1 = (SELECT AVG(product_cost)
                        FROM product1_account, product 
                        WHERE product1_account.product_id=product.id AND YEAR(product.date_produced)=@year);
    ...
    IF ((SELECT AVG(product_cost)
                        FROM product2_account, product 
                        WHERE product2_account.product_id=product.id AND YEAR(product.date_produced)=@year) is not null)
    BEGIN
    SET @avg_cost2 = (SELECT AVG(product_cost)
                        FROM product2_account, product 
                        WHERE product2_account.product_id=product.id AND YEAR(product.date_produced)=@year);
    ...
    IF ((SELECT AVG(product_cost)
                        FROM product3_account, product 
                        WHERE product3_account.product_id=product.id AND YEAR(product.date_produced)=@year) is not null)
    BEGIN
    SET @avg_cost3 = (SELECT AVG(product_cost)
                        FROM product3_account, product 
                        WHERE product3_account.product_id=product.id AND YEAR(product.date_produced)=@year);
...
--a procedure for deleting a product
/*CREATE PROCEDURE delete_product
    @id varchar(20)
AS
BEGIN
     IF ((@id not in (select product.id
                from product)))
        BEGIN
            PRINT 'No product with thid ID was found.'
            RETURN -1
        END
    DELETE FROM product
    WHERE product.id = @id;

    IF(@id in (select product1.id from product1))
        BEGIN
            DELETE FROM product1 WHERE product1.id = @id;
        END
    IF(@id in (select product2.id from product2))
        BEGIN
            DELETE FROM product2 WHERE product2.id = @id;
        END
    IF(@id in (select product3.id from product3))
        BEGIN
            DELETE FROM product3 WHERE product3.id = @id;
        END
END */
